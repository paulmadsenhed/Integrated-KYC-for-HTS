...
3. The application queries the account to determine if the KYC status flag has been set
    4. If the flag is set, the Verifier requests a KYC VC from the user
    5. If the flag is not set, the Verifier redirects the User to the relevant KYC Provider for that token 
